                ...
                :class="[
                  ...
                  },
                  {
                    'between-selected':
                      mode === 'range' &&
                      selectData.length > 0 &&
                      new Date(selectData[0].date) < new Date(it.date) &&
                      selectData.length > 1 &&
                      new Date(selectData[1].date) > new Date(it.date)
                  },
                  {
                    selected:
                      mode === 'range' &&
                      ((selectData.length > 0 &&
                        selectData[0].date === it.date) ||
                        (selectData.length > 1 &&
                          selectData[1].date === it.date))
                  },
                  { 'hide-item': it.type !== 'normal' },
                  {
                    'grey-item':
                      new Date(it.date) < new Date(minDate) ||
                      new Date(it.date) > new Date(maxDate)
                  }
                ]"
                @click="checkoutDate(it, index)"
              >
                <span :class="['data-font', 'calendar-item']">{{
                  it.day
                }}</span>
                <span
                  class="block"
  ...
  methods: {
    ...
    */
    ...
    checkoutDate(selectData) {
      if (
        new Date(selectData.date) < new Date(this.minDate) ||
        new Date(selectData.date) > new Date(this.maxDate)
      ) {
        return;
      }
      let tempArray = [];
      ...
      switch (this.mode) {
        ...
        case "range":
          if (
            !(
              tempArray.length == 1 &&
              new Date(tempArray[0].date) < new Date(selectData.date)
            )
          ) {
            tempArray = [];
          }
